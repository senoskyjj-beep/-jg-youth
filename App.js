...
// Two main leaders, each with their own PIN. Both have full admin rights.
// Add or change here. Names appear in admin "Logged in as" header.
var MAIN_LEADERS = [
  { pin: "91977", name: "Joshua" },
  { pin: "91976", name: "Priscilla" }
];
// Backwards-compat: legacy code still references ADMIN_PIN. Points to Joshua's PIN.
var ADMIN_PIN  = "91977";
var WA_GROUP   = "https://chat.whatsapp.com/EfcayPaEFQd2tA7PukASuK?mode=gi_t";
// Shared secret. Must match SYNC_TOKEN in GoogleAppsScript.js. Rotate this periodically
// (change here AND in the Apps Script, redeploy the script). This is not perfect security
// since it ships in the JS bundle, but it prevents random people who find the URL from
// writing to the sheet.
var SYNC_TOKEN = "JG2026LiveWaters";

// Helper: returns the main leader entry if the PIN matches one of MAIN_LEADERS, else null.
function getMainLeader(pin){
  for(var i=0;i<MAIN_LEADERS.length;i++){ if(MAIN_LEADERS[i].pin===pin)return MAIN_LEADERS[i]; }
  return null;
}
function isMainPin(pin){ return !!getMainLeader(pin); }
...
function saveData(d) {
  ...
  try {
    ...
    (d.members||[]).forEach(function(m){
      if(m.photo&&m.photo!=="HAS_PHOTO"&&m.id){
        try{ localStorage.setItem("ph_"+m.id,m.photo); }
        catch(e){ photoErrors.push(m.name+" "+m.surname); }
      }
    });
    if(photoErrors.length>0){
      console.warn("Photo storage failed (likely quota exceeded) for:", photoErrors);
      // Surface to the UI so the user knows their photos didn't save
      try{
        window.dispatchEvent(new CustomEvent("jg-storage-warning",{detail:{
          type:"photo-quota",
          names:photoErrors,
          message:"Some photos could not be saved (browser storage full). Affected: "+photoErrors.join(", ")
        }}));
      }catch(_){}
    }
  } catch(e) {
    ...
    try{
      window.dispatchEvent(new CustomEvent("jg-storage-warning",{detail:{
        type:"save-failed",
        message:"Could not save data locally: "+(e.message||"unknown error")
      }}));
    }catch(_){}
  }
}
...
async function syncGoogle(payload){
  if(!GOOGLE_URL||GOOGLE_URL.includes("PASTE"))return {ok:false, reason:"not-configured"};
  // Use text/plain to avoid CORS preflight (Apps Script doesn't handle OPTIONS well).
  // e.postData.contents on the server side still contains the raw JSON body.
  // Token sent in body so it's not in the URL/referer logs.
  ...
  try{
    ...
    if(data.status !== "ok"){
      console.warn("Sync rejected:", data.message||"unknown");
      return {ok:false, reason:data.message||"server-rejected"};
    }
    return {ok:true};
  }catch(e){
  ...
  function press(k){
    ...
    if(next.length===5){
      ...
      if(main){
        ...
        // Junior leaders cannot enter the admin portal (only home tile access)
...
function RegistrationForm({existingMembers,onDone,onBack,prefill}){
  ...
  return(<div>
    ...
    <div style={{marginBottom:12}}>
      <label style={{display:"block",fontSize:13,fontWeight:600,color:"#94a3b8",marginBottom:4}}>Grade</label>
      <select className="input" value={form.grade||""} style={{background:"#1e293b",color:"#fff"}}
        ...
        onChange={function(e){
          if(e.target.files[0]){
            ...
            reader.onload=function(ev){
              var img=new Image();
              img.onload=function(){
                // Downscale to 800x800 max, keeping aspect ratio. This keeps
                // photos under ~50KB for fast Drive sync and small bandwidth.
                var maxDim=800;
                ...
                // Camera capture from <input capture="user"> already returns an
                // unmirrored image. Drawing it as-is gives the natural orientation.
...
function AdminDashboardEmbedded({data,setData,initialTab}){
  ...
  function deleteMember(id){
    ...
    if(role==="senior"){alert("⚠️ Senior leaders cannot delete members.\n\nOnly Main Leaders (Joshua or Priscilla) can delete from the registration list. Please ask her if a member needs to be removed.");return;}
    var m=members.find(function(mb){return mb.id===id;});
    if(!m)return;
      ...
      {allVisitors.map(function(m){
        ...
        return(<div key={m.id} style={{background:"#1e293b",border:"2px solid #a855f722",borderRadius:13,padding:"14px 16px",marginBottom:10}}>
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:10}}>
            {toWA(m.whatsapp||m.phone)&&<a href={toWA(m.whatsapp||m.phone,msgVisitor(m.name))} target="_blank" style={{background:"#a855f7",color:"#fff",borderRadius:9,padding:"8px 14px",fontSize:13,fontWeight:700,textDecoration:"none"}}>💜 Thank You WA</a>}
            ...
            {toWA(m.whatsapp||m.phone)&&<a href={toWA(m.whatsapp||m.phone)} target="_blank" className="btn btn-wa">💬 Chat</a>}
            {toSMS(m.phone)&&<a href={toSMS(m.phone)} style={{background:"#0891b2",color:"#fff",borderRadius:9,padding:"8px 14px",fontSize:13,fontWeight:700,textDecoration:"none"}}>📱 SMS</a>}
          </div>
        </div>);
      })}
    </div>
    ...
    <div>
      ...
      {sortAlpha(members).map(function(m,i){return(<div key={m.id||i} style={{background:i%2===0?"#1e293b":"#182032",borderRadius:12,padding:"12px 14px",marginBottom:6,display:"flex",flexWrap:"wrap",alignItems:"center",gap:10}}>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          <a href={toWA(m.whatsapp||m.phone)} target="_blank" className="btn btn-wa">💬 WA</a>
          {toSMS(m.phone)&&<a href={toSMS(m.phone)} className="btn" style={{background:"#0891b2",color:"#fff",borderRadius:9,padding:"7px 10px",fontSize:12,fontWeight:700,textDecoration:"none"}}>📱 SMS</a>}
          <a href={toWA(m.parentPhone)} target="_blank" className="btn btn-wa-parent">👨‍👩‍👦 WA</a>
          {toSMS(m.parentPhone)&&<a href={toSMS(m.parentPhone)} className="btn" style={{background:"#7c2d12",color:"#fff",borderRadius:9,padding:"7px 10px",fontSize:12,fontWeight:700,textDecoration:"none"}}>📱 SMS</a>}
        </div>
      </div>);})}
    </div>
  );
...
function AdminDashboard({data,setData,onExit,onRefresh,syncing}){
  ...
  return(<div>
    {(function(){
      ...
      var mainName=localStorage.getItem("jg_admin_main_name")||"";
      var lname="";
      if(lid){
        var L=JSON.parse(localStorage.getItem("jg_leaders")||"[]").find(function(x){return x.id===lid;});
        if(L)lname=L.name+" "+L.surname;
      }
      if(role==="master")return(<div style={{background:"linear-gradient(90deg,#6c63ff,#3b82f6)",color:"#fff",textAlign:"center",padding:"8px 12px",fontSize:12,fontWeight:700,marginBottom:10,borderRadius:8}}>✝ Main Leader: {mainName||"Admin"} · Full access</div>);
      if(role==="senior")return(<div style={{background:"linear-gradient(90deg,#fbbf24,#f59e0b)",color:"#000",textAlign:"center",padding:"8px 12px",fontSize:12,fontWeight:700,marginBottom:10,borderRadius:8}}>⭐ Senior Leader: {lname} · You can message & view reports. Cannot delete (ask a Main Leader).</div>);
      return null;
    })()}
    <div style={{textAlign:"center",marginBottom:14}}>
    ...
    {tab==="absent"&&<div>
      ...
      {lastDate!==todayDate&&<div>
        ...
        {absentOn(lastDate).map(function(m){return(<div key={m.id} style={{background:"#1e293b",borderRadius:12,padding:"12px 14px",marginBottom:9,display:"flex",flexWrap:"wrap",alignItems:"center",gap:10}}>
          <div style={{flex:1}}><strong>{m.name} {m.surname}</strong><div style={{fontSize:13,color:"#94a3b8"}}>Phone: {m.phone||"?"}{calcAge(m.birthday)!==null?" · Age "+calcAge(m.birthday):""} | Parent: {m.parentPhone||"?"}</div></div>
          <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
            {toWA(m.whatsapp||m.phone)&&<a href={toWA(m.whatsapp||m.phone)} target="_blank" className="btn btn-wa">💬 WA</a>}
            ...
            {toWA(m.whatsapp||m.phone)&&<a href={toWA(m.whatsapp||m.phone,msgAbsent(m.name))} target="_blank" style={{background:"#6c63ff",color:"#fff",borderRadius:9,padding:"7px 10px",fontSize:12,fontWeight:700,textDecoration:"none"}}>💜 Encourage</a>}
            {toSMS(m.phone)&&<a href={toSMS(m.phone,msgAbsent(m.name))} style={{background:"#4f46e5",color:"#fff",borderRadius:9,padding:"7px 10px",fontSize:12,fontWeight:700,textDecoration:"none"}}>📱 Encourage</a>}
            {toWA(m.parentPhone)&&<a href={toWA(m.parentPhone,msgParentAbsent(m.name,m.parentName))} target="_blank" style={{background:"#be185d",color:"#fff",borderRadius:9,padding:"7px 10px",fontSize:12,fontWeight:700,textDecoration:"none"}}>❤️ Parent WA</a>}
            {toSMS(m.parentPhone)&&<a href={toSMS(m.parentPhone,msgParentAbsent(m.name,m.parentName))} style={{background:"#7c2d12",color:"#fff",borderRadius:9,padding:"7px 10px",fontSize:12,fontWeight:700,textDecoration:"none"}}>📱 Parent SMS</a>}
          </div>
        </div>);})}
      </div>}
    </div>}
    ...
    {tab==="contacts"&&<div>
      ...
      {sortAlpha(members).map(function(m,i){return(<div key={m.id||i} style={{background:i%2===0?"#1e293b":"#182032",borderRadius:12,padding:"12px 14px",marginBottom:8,display:"flex",flexWrap:"wrap",alignItems:"center",gap:10}}>
        ...
        <div style={{flex:1,minWidth:140}}>
          <strong>{m.name} {m.surname}</strong>
          <div style={{fontSize:12,color:"#94a3b8"}}>Phone: {m.phone||"?"}{calcAge(m.birthday)!==null?" · Age: "+calcAge(m.birthday):""}</div>
          <div style={{fontSize:12,color:"#94a3b8"}}>Parent: {m.parentName||""} {m.parentSurname||""} | {m.parentPhone||"?"}</div>
        </div>
        <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
          {toWA(m.whatsapp||m.phone)&&<a href={toWA(m.whatsapp||m.phone)} target="_blank" className="btn btn-wa">💬 WA</a>}
          {toSMS(m.phone)&&<a href={toSMS(m.phone)} style={{background:"#0891b2",color:"#fff",borderRadius:9,padding:"7px 10px",fontSize:12,fontWeight:700,textDecoration:"none"}}>📱 SMS</a>}
          {toWA(m.parentPhone)&&<a href={toWA(m.parentPhone)} target="_blank" className="btn btn-wa-parent">Parent WA</a>}
          {toSMS(m.parentPhone)&&<a href={toSMS(m.parentPhone)} style={{background:"#7c2d12",color:"#fff",borderRadius:9,padding:"7px 10px",fontSize:12,fontWeight:700,textDecoration:"none"}}>📱 SMS</a>}
        </div>
      {sortAlpha(members).map(function(m){
        ...
        return(<div key={m.id} style={{background:"#1e293b",borderRadius:14,padding:"14px 16px",marginBottom:12,border:wk>=3?"2px solid #ef444444":"2px solid transparent"}}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            ...
            {toWA(m.parentPhone)&&<a href={toWA(m.parentPhone)} target="_blank" className="btn btn-wa-parent">Parent WA</a>}
            {toSMS(m.parentPhone)&&<a href={toSMS(m.parentPhone)} style={{background:"#7c2d12",color:"#fff",borderRadius:9,padding:"7px 10px",fontSize:12,fontWeight:700,textDecoration:"none"}}>📱 SMS</a>}
            {wk>=3&&toWA(m.whatsapp||m.phone)&&<a href={toWA(m.whatsapp||m.phone,msgAbsent(m.name))} target="_blank" style={{background:"#6c63ff",color:"#fff",borderRadius:9,padding:"7px 12px",fontSize:12,fontWeight:700,textDecoration:"none"}}>💜 Encourage</a>}
            {wk>=3&&toSMS(m.phone)&&<a href={toSMS(m.phone,msgAbsent(m.name))} style={{background:"#4f46e5",color:"#fff",borderRadius:9,padding:"7px 12px",fontSize:12,fontWeight:700,textDecoration:"none"}}>📱 Encourage</a>}
            {wk>=3&&toWA(m.parentPhone)&&<a href={toWA(m.parentPhone,msgParentAbsent(m.name,m.parentName))} target="_blank" style={{background:"#be185d",color:"#fff",borderRadius:9,padding:"7px 12px",fontSize:12,fontWeight:700,textDecoration:"none"}}>❤️ Parent WA</a>}
            {wk>=3&&toSMS(m.parentPhone)&&<a href={toSMS(m.parentPhone,msgParentAbsent(m.name,m.parentName))} style={{background:"#7c2d12",color:"#fff",borderRadius:9,padding:"7px 12px",fontSize:12,fontWeight:700,textDecoration:"none"}}>📱 Parent SMS</a>}
          </div>
        </div>);
      })}
    </div>}
          ...
          {absentList.length===0?<p className="green" style={{margin:0}}>Full attendance!</p>
            :absentList.map(function(m){
              ...
              return(<div key={m.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:"1px solid #334155",flexWrap:"wrap",gap:8}}>
                ...
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                  {toWA(m.whatsapp||m.phone)&&<a href={toWA(m.whatsapp||m.phone)} target="_blank" className="btn btn-wa" style={{fontSize:11,padding:"4px 8px"}}>💬</a>}
                  ...
                  {toWA(m.parentPhone)&&<a href={toWA(m.parentPhone)} target="_blank" className="btn btn-wa-parent" style={{fontSize:11,padding:"4px 8px"}}>Parent 💬</a>}
                  {toSMS(m.parentPhone)&&<a href={toSMS(m.parentPhone)} style={{background:"#7c2d12",color:"#fff",borderRadius:7,padding:"4px 8px",fontSize:11,fontWeight:700,textDecoration:"none"}}>Parent 📱</a>}
                  {wk>=3&&toWA(m.whatsapp||m.phone)&&<a href={toWA(m.whatsapp||m.phone,msgAbsent(m.name))} target="_blank" style={{background:"#6c63ff",color:"#fff",borderRadius:7,padding:"4px 8px",fontSize:11,fontWeight:700,textDecoration:"none"}}>💜</a>}
                  {wk>=3&&toSMS(m.phone)&&<a href={toSMS(m.phone,msgAbsent(m.name))} style={{background:"#4f46e5",color:"#fff",borderRadius:7,padding:"4px 8px",fontSize:11,fontWeight:700,textDecoration:"none"}}>📱💜</a>}
                  {wk>=3&&toWA(m.parentPhone)&&<a href={toWA(m.parentPhone,msgParentAbsent(m.name,m.parentName))} target="_blank" style={{background:"#be185d",color:"#fff",borderRadius:7,padding:"4px 8px",fontSize:11,fontWeight:700,textDecoration:"none"}}>❤️</a>}
                  {wk>=3&&toSMS(m.parentPhone)&&<a href={toSMS(m.parentPhone,msgParentAbsent(m.name,m.parentName))} style={{background:"#7c2d12",color:"#fff",borderRadius:7,padding:"4px 8px",fontSize:11,fontWeight:700,textDecoration:"none"}}>❤️📱</a>}
                </div>
              </div>);
            })
          }
...
function LeadersTab(){
  ...
  function nextPin(role){
    // Senior leaders use prefix 2026 (PINs 20261-20269).
    // Junior leaders use prefix 3026 (PINs 30261-30269).
    // 9 slots per role keeps PINs to 5 digits (the input pad accepts exactly 5).
    var prefix = role==="Senior" ? "2026" : "3026";
    // Look at existing PINs with this prefix
    var existing=leaders
      .filter(function(L){return L.pin && L.pin.indexOf(prefix)===0;})
      .map(function(L){return parseInt(L.pin.slice(4))||0;});
  ...
  function saveLeader(){
    ...
    if(editing){
      ...
      var pin=nextPin(form.role);
      if(pin==="FULL"){
        alert("All 9 "+form.role+" Leader PIN slots are in use. Remove an inactive "+form.role+" Leader before adding a new one.");
        return;
      }
      var newLeader=Object.assign({},form,{id:"L"+Date.now(),pin:pin,addedOn:todayStr()});
  ...
  if(showForm){
    return(<div>
      ...
      <div style={{background:"#1e293b",borderRadius:13,padding:"16px"}}>
            ...
            if(e.target.files[0]){
              ...
              reader.onload=function(ev){
                ...
                img.onload=function(){
                  ...
                  canvas.width=w; canvas.height=h;
                  var ctx=canvas.getContext("2d");
                  // Camera capture is already unmirrored; draw as-is.
                  ctx.drawImage(img,0,0,w,h);
        ...
        {!editing&&(function(){
          var p=nextPin(form.role);
          if(p==="FULL")return <p style={{color:"#f87171",fontSize:13,marginBottom:10,fontWeight:700}}>⚠️ All 9 {form.role} Leader slots are full. Remove a {form.role} Leader before adding a new one.</p>;
          return <p style={{color:"#a5b4fc",fontSize:13,marginBottom:10}}>Auto-generated PIN: <strong>{p}</strong> ({form.role} Leader)</p>;
        })()}
...
function App(){
  ...
  // Toast for storage warnings (e.g. photo quota exceeded). Auto-clears after 8s.
  var [toast,setToast]=useState(null);
  // Triple-tap on logo → admin. tapCount resets after 1.5s of no taps.
  var [tapCount,setTapCount]=useState(0);
  var tapTimerRef=useRef(null);

  function checkHomeTilePin(p){
    var leaders=JSON.parse(localStorage.getItem("jg_leaders")||"[]");
    // Main leaders, Senior leaders, AND Junior leaders all unlock home tiles
    var anyLeader=leaders.find(function(L){return L.pin===p;});
    if(isMainPin(p)||anyLeader){
  ...
  // Listen for storage-warning events dispatched by saveData and show a toast.
  useEffect(function(){
    function onWarn(e){
      var msg=(e&&e.detail&&e.detail.message)||"Could not save some data locally.";
      setToast({kind:"warn",msg:msg});
      setTimeout(function(){setToast(null);},8000);
    }
    window.addEventListener("jg-storage-warning",onWarn);
    return function(){window.removeEventListener("jg-storage-warning",onWarn);};
  },[]);

  useEffect(function(){
    loadFromGoogle();
    // Auto-refresh every 30 seconds so all devices stay in sync.
    // BUT skip refresh while the user is in registration or check-in to avoid
    // overwriting their in-progress local state with stale Sheet data.
  ...
  async function loadFromGoogle(){
    ...
    try{
      // Token sent as URL param so Apps Script can validate before returning data
      // (which now includes photos).
      ...
      if(json.status==="ok"){
        ...
        // Photos: prefer the photo from Drive (returned by the script); fall back
        // to local photo if Drive returned null (e.g. file deleted, or member
        // registered before Drive integration).
  ...
  function handleRegistered(member,isNew){
    ...
    if(isNew){
      // Send photo through to Apps Script — script saves it to the JG Photos
      // Drive folder and stores the file ID in the Members sheet.
  ...
  // Triple-tap handler — counts taps on the logo/banner. On 3rd tap in < 1.5s
  // navigates to the hidden PIN screen. No visible indicator is shown so public
  // users have no idea the admin panel exists.
  function handleLogoBannerTap(){
    ...
    if(next>=3){
      ...
      setTapCount(next);
      tapTimerRef.current=setTimeout(function(){setTapCount(0);},1500);
    }
  }

  // Toast renderer used on every screen
  function ToastBar(){
    if(!toast)return null;
    return(<div style={{position:"fixed",top:14,left:"50%",transform:"translateX(-50%)",zIndex:99999,maxWidth:"92%"}}>
      <div style={{background:toast.kind==="warn"?"#7c2d12":"#1e293b",color:"#fff",padding:"10px 16px",borderRadius:10,fontSize:13,fontWeight:600,boxShadow:"0 4px 18px rgba(0,0,0,0.6)",border:"1px solid "+(toast.kind==="warn"?"#f59e0b":"#334155")}}>
        ⚠️ {toast.msg} <span onClick={function(){setToast(null);}} style={{marginLeft:10,cursor:"pointer",opacity:0.7}}>✕</span>
      </div>
    </div>);
  }

  if(screen==="register")return(<><ToastBar/><div className="container"><RegistrationForm existingMembers={data.members||[]} prefill={prefill} onDone={function(m,isNew){setPrefill(null);handleRegistered(m,isNew);setScreen("home");}} onBack={function(){setPrefill(null);setScreen("home");}}/></div></>);
  if(screen==="checkin")return(<><ToastBar/><div className="container"><CheckInPage members={data.members||[]} checkins={data.checkins||[]} onCheckin={handleCheckin} onBack={function(){setScreen("home");}} onCompleteProfile={function(m){setPrefill(m);setScreen("register");}}/></div></>);
  if(screen==="pin")return(<><ToastBar/><PinScreen onSuccess={function(){setScreen("admin");}}/></>);
  if(screen==="admin")return(<><ToastBar/><div className="container"><AdminDashboard data={data} setData={function(d){setData(d);saveData(d);}} onExit={function(){localStorage.removeItem("jg_admin_role");localStorage.removeItem("jg_admin_leader_id");localStorage.removeItem("jg_admin_main_name");setScreen("home");}} onRefresh={loadFromGoogle} syncing={syncing}/></div></>);

  return(<><ToastBar/><div style={{minHeight:"100vh",background:"#000",display:"flex",flexDirection:"column"}}>
    {/* Banner Image Hero */}
    <div style={{position:"relative",width:"100%",overflow:"hidden"}}>
      <img src="data:image/jpeg;base64," alt="Jeremiah Generation"
        ...
        onClick={handleLogoBannerTap}
        onTouchStart={function(e){e.preventDefault();handleLogoBannerTap();}}
        />
    ...
    <div style={{flex:1,padding:"20px 16px 32px",display:"flex",flexDirection:"column",alignItems:"center"}}>
      ...
      <p style={{color:"#334155",fontSize:11,marginTop:20,textAlign:"center"}}>
        Jeremiah Generation · Living Waters Fellowship
      </p>
    </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,maxWidth:280,margin:"0 auto"}}>
                  {["1","2","3","4","5","6","7","8","9","","0","del"].map(function(k,i){
                    ...
                    return <button key={i} onClick={function(){
                      ...
                      setHomeTilePin(next);
                      // Both master and leader PINs are 5 digits; check on completion.
                  ...
                  return(<div key={m.id} style={{background:"#1e293b",borderRadius:12,padding:"10px 12px",marginBottom:6}}>
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        {m.phone&&<div style={{fontSize:11,color:"#64748b"}}>📞 {m.phone}{calcAge(m.birthday)!==null?" · Age "+calcAge(m.birthday):""}</div>}
