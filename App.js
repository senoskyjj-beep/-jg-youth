...
function withPhotos(d) {
  return Object.assign({},d,{members:(d.members||[]).filter(function(m){return m&&m.id!=null;}).map(function(m){
    if(m.id){
      ...
      if(p && p.indexOf("data:")===0) return Object.assign({},m,{photo:p});      // still on this phone (not uploaded yet)
      if(m.photoUrl) return Object.assign({},m,{photo:driveImg(m.photoUrl)});     // uploaded — load it FROM Google Drive
    }
    if(m.photo==="HAS_PHOTO") return Object.assign({},m,{photo:null});
    return m;
  })});
}
...
function pruneUploadedPhotos(members){
  (members||[]).forEach(function(m){
    ...
    if(pu.indexOf("drive.google")>=0 || pu.indexOf("googleusercontent")>=0){
      try{ localStorage.removeItem("ph_"+m.id); }catch(e){}
    }
  });
}
...
async function uploadPendingPhoto(item,members){
  ...
  try{
    ...
    if(json.status==="ok"&&json.photoUrl){
      ...
      try{ localStorage.removeItem("ph_"+memberId); }catch(e){}
      return true;
    }
    return false;
  }catch(e){
...
async function postToGoogle(payload){
  ...
  try{
    ...
    if(timer)clearTimeout(timer);
    try{ var json=await r.json(); return !!(json&&json.status==="ok"); }catch(e2){ return r.ok; }
  }catch(e){
    if(timer)clearTimeout(timer);
...
async function uploadPhotoData(member,photoBase64){
  ...
  try{
    ...
    if(json.status==="ok"&&json.photoUrl){
      ...
      try{ localStorage.removeItem("ph_"+member.id); }catch(e){}
      return true;
    }
  }catch(e){}
  return false;
...
function AdminDashboard({data,setData,onExit,onRefresh,syncing}){
  ...
  return(<div>
    ...
    {(function(){
      function tabById(id){return tabs.find(function(t){return t.id===id;})||{};}
      var ADMIN_GROUPS=[
        {id:"absent",label:"Absent Today",emoji:"⚠️",tabIds:["absent","sent"],desc:"Message absentees and see who's been contacted"},
        {id:"people",label:"People",emoji:"👥",tabIds:["members","visitors","contacts"],desc:"Members, visitors and contacts"},
        {id:"leaders",label:"Leaders",emoji:"⭐",tabIds:["leaders","leader_attendance","qr","reset"],desc:"PINs, attendance, QR check-in, reset"},
        {id:"events",label:"Special Events",emoji:"📅",tabIds:["events"],desc:"Create events, attach a poster, send invites"},
        {id:"comms",label:"Communication",emoji:"💬",tabIds:["suggestions","wagroup"],desc:"Suggestions inbox and WhatsApp group requests",badge:unreadFb+waRequests.length},
        {id:"data",label:"Data",emoji:"📊",tabIds:["reports","sheet","export","import","pending"],desc:"Reports, spreadsheet, export/import, pending sync",badge:loadPendingQueue().length},
      ];
      if(adminGroup){
        var g=ADMIN_GROUPS.find(function(x){return x.id===adminGroup;})||{tabIds:[],label:""};
        return(<div style={{position:"fixed",top:0,left:0,right:0,zIndex:310,background:"var(--jg-bg)",padding:"16px 16px 12px",boxShadow:"0 4px 14px rgba(0,0,0,0.25)"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:g.tabIds.length>1?12:0,paddingRight:56}}>
            <h3 className="page-title" style={{margin:0}}>{g.label}</h3>
            <button onClick={closeGroup} style={{width:36,height:36,borderRadius:12,background:"var(--jg-card)",border:"none",color:"var(--jg-text)",fontSize:16,fontWeight:800,cursor:"pointer"}}>✕</button>
          </div>
          {g.tabIds.length>1&&<div className="tabs" style={{marginBottom:0}}>
            {g.tabIds.map(function(tid){var t=tabById(tid);return(<button key={tid} className={"tab"+(tab===tid?" active":"")} onClick={function(){setTab(tid);}} style={{position:"relative"}}>
              {t.label}{t.badge>0&&<span className="badge">{t.badge}</span>}
            </button>);})}
          </div>}
        </div>);
      }
      var restGroups=ADMIN_GROUPS.filter(function(g){return g.id!=="absent";});
      return(<div style={{marginBottom:20}}>
        <button onClick={function(){openGroup("absent","absent");}} style={{width:"100%",textAlign:"center",background:"linear-gradient(135deg,#ef4444,#f59e0b)",border:"none",borderRadius:20,padding:"20px",cursor:"pointer",fontFamily:"inherit",marginBottom:12,boxShadow:"0 8px 24px rgba(239,68,68,0.35)"}}>
          <div style={{fontSize:12,fontWeight:700,color:"rgba(255,255,255,0.85)",letterSpacing:"1.5px",textTransform:"uppercase",marginBottom:6}}>Absent Today</div>
          <div style={{fontSize:44,fontWeight:900,color:"#fff",lineHeight:1}}>{absentToday.length}</div>
          <div style={{fontSize:13,fontWeight:700,color:"rgba(255,255,255,0.9)",marginTop:2}}>of {members.length} did not attend</div>
          <div style={{fontSize:12,color:"rgba(255,255,255,0.85)",marginTop:8,fontWeight:600}}>Tap to see who and follow up</div>
        </button>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {restGroups.map(function(g){return(
            <button key={g.id} onClick={function(){openGroup(g.id,g.tabIds[0]);}} style={{textAlign:"left",background:"var(--jg-card)",border:"none",borderRadius:16,padding:"14px",cursor:"pointer",fontFamily:"inherit",position:"relative"}}>
              <div style={{fontSize:22,marginBottom:6}}>{g.emoji}</div>
              <div style={{fontSize:14,fontWeight:800,color:"var(--jg-text)",marginBottom:2}}>{g.label}</div>
              <div style={{fontSize:11,color:"var(--jg-muted)",lineHeight:1.3}}>{g.desc}</div>
              {g.badge>0&&<span className="badge" style={{position:"absolute",top:10,right:10}}>{g.badge}</span>}
            </button>
          );})}
        </div>
      </div>);
    ...
    {!adminGroup&&tab==="overview"&&<div>
      ...
      {absentToday.length>0&&<div style={{background:"var(--jg-card)",border:"2px solid #f59e0b44",borderRadius:13,padding:"14px 16px",marginBottom:14}}>
        ...
        <button className="btn btn-check" style={{width:"auto",padding:"9px 18px",fontSize:14}} onClick={function(){setAdminGroup("absent");setTab("absent");}}>See Absent List and WhatsApp</button>
      </div>}
    ...
    {adminGroup&&<div style={{position:"fixed",inset:0,zIndex:240,background:"var(--jg-bg)",overflowY:"auto",padding:"76px 16px 30px"}}>
